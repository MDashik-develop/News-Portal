                ...
                @foreach ($reels as $reel)
                    ...
                    @if ($videoId)
                        <article id="reel-{{ $reel->id }}" 
                                 ...
                                 data-avatar="{{ asset('storage/' . $reel->user->profile_image) }}"
                                 data-slug="{{ $reel->slug }}">
                            ...
                            <div class="reel-info absolute bottom-0 left-0 p-4 w-full bg-gradient-to-t from-black/70 to-transparent transition-opacity duration-300 pointer-events-none">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div class="max-w-10 min-w-10 h-10 rounded-full overflow-hidden border-2 border-white">
                                        <img src="{{ asset('storage/' . $reel->user->profile_image) }}" alt="Reporter Avatar" class="w-full min-h-full">
                                    </div>
                                    <div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const initializeReels = () => {
                ...
                const handleMouseEnter = (e) => {
                    ...
                    placeholder.innerHTML = `<iframe class="reel-iframe pointer-events-none" src="https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&controls=0&loop=1&playlist=${videoId}" allow="autoplay" allowfullscreen></iframe>`;
                };
